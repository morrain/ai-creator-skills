...
- **四大平台 (小红书/微信视频号/抖音/快手) 深度适配标题矩阵与字数约束 (共 8 个标题，一票否决)**：检查标题备选矩阵是否单独列在最顶部。**顶部必须生成 8 个备选标题**：
  1) 第 1 个备选标题必须严格为母版正文大标题 H1（固定首位，无需标注字数）；
  2) 四大平台字数与算法适配结构必须包含：
     - **小红书爆款规格 (至少 2 个 ≤ 20 字)**：前 15 字前置核心 SEO 关键词与情绪切入点；
     - **微信视频号/公众号规格 (至少 2 个 ≤ 16 字)**：单行无折叠精炼标题，突出专业深度与定性；
     - **抖音爆款规格 (至少 2 个 ≤ 30 字)**：前 12 字前置痛点冲突/反常识悬念钩子；
     - **快手落地实用规格 (至少 1 个 ≤ 25 字)**：保姆级实用技巧与避坑拆弹指北；
  3) 在满足字数限制的前提下，标题字数要尽可能多、尽可能完整表达原文核心意思；
  4) **除了第 1 个固定列举的文章 H1 标题外，其它所有备选标题必须在标题末尾使用括号显式标注字数**（如 `示例爆款标题文本 (18字)`）。
  若标题少于 8 个、不满足四大平台字数/算法适配结构、缺乏原文意思表达，或除了第 1 个标题外未在标题末尾加括号标注字数，直接裁决为 `[REJECT]`。
...
- **6-10 个热度与搜索联动话题标签 (一票否决)**：文案最末尾**必须包含 6-10 个以 `#` 开头的相关话题标签**。拟定标签时必须深度结合母版主题与平台当前热点，并严格遵循“顶流热词 ➔ 垂直高频词 ➔ 精准概念词”的三层金字塔降序结构（如 `#AI大模型 #人工智能 #LLM原理 #Agent架构 #状态机设计`）。若少于 6 个或缺乏标签，或使用了 `#科普 #干货 #冷知识 #自媒体选题 #爆款指南 #知识点` 等泛化废话标签，或未按三层金字塔降序排列，直接裁决打回 `[REJECT]`。

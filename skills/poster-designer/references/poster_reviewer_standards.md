...
- **原文长文阅读引导提示**：海报（特别是组图封底页或卡片 Footer 区域）宜包含引导读者阅读原文长文的手绘胶囊贴纸与中文文本（如 `'🔗 点击阅读完整原文'`）。
...
- **原文长文跳转引导句 (一票否决)**：在正文结尾、话题标签之前**必须包含 1-2 行醒目的原文长文阅读引导提示句**（如 `👇 想要解锁完整深度拆解？点击下方【阅读原文】链接 🔗`），格式绝对保持纯文本。若缺少原文引导提示，直接裁决打回 `[REJECT]`。

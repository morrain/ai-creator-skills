...
- **标题矩阵数量与字数结构约束 (共 8 个标题：3 个 ≤ 20 字 + 1 个 ≤ 16 字，一票否决)**：检查标题备选矩阵是否单独列在最顶部，第一备选标题必须为母版正文大标题 H1。**顶部必须生成 8 个备选标题**，且字数结构必须包含 **至少 3 个不超过 20 字的标题** 以及 **至少 1 个不超过 16 字的短标题**（其余 4 个标题字数不限制）。若标题总数少于 8 个或不满足上述字数结构约束，直接裁决为 `[REJECT]`。

...
5. 🚫 **浅色画布标题防突兀黑框禁令**: 当 Canvas 为浅色渐变时，标题组（`#title-group`）必须采用透明无框透底，主标题直接使用深色高对比文字 `fill="#0f172a"`，副标题使用主题主色 `fill="#2563eb"`，绝对禁止在标题后方放置深色 `<rect fill="#0f172a">` 胶囊框/黑卡片，彻底消除画面顶部突兀黑块。

...
5. 存盘写入 `./<article-slug>/assets/video/unit_XX/BRIEF.md`，各板块要求如下：
   ...
   **`## Notes` 板块**（强制写入以下全部规程）：
   ...
   > `- 9:16 视频平台 (小红书/抖音/视频号) 顶部 UI 避让留白规程：针对 9:16 竖屏，顶部 Y: 0px - 120px (至少 120px+) 必须保留为纯净背景避让留白区，绝对禁止放置任何标题文字、实体构件或图标，防止发布后被小红书/抖音的返回按钮、搜索键及手机刘海/状态栏遮挡！顶部标题区起始 Y 必须从 120px 开始。`

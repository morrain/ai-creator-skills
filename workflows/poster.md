...
1. **社媒文案合成 (`poster_post.md`)**：
   - 📌 **标题顶部独立矩阵**：在文案最顶部单独列出 3-5 个爆款备选标题。**必须显式将原文正文大标题 (Master Article H1) 作为第一备选项**，严禁漏掉母版长文的原始标题。
...
2. **阶段三 SubAgent 文案审稿与落盘**：
   - 显式调用 `invoke_subagent` 发起 `poster_reviewer` 子进程，读取并执行 `skills/poster-designer/references/poster_reviewer_standards.md` 审查 `poster_post.md` 的顶部母版 H1 标题、100 字限制、纯文本隔离度与 6-10 个 `#话题标签` 完整性，修正直至 `[PASS]`，落盘至 `./<article-slug>/poster_post.md`。
3. **结构化汇报与懒加载提示**：

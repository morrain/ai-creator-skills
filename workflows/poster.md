...
1. **社媒文案合成 (`poster_post.md`)**：
   ...
   - 🏷️ **6-10 个结合主题与当下热度降序排列的话题标签 (Topic Tags with Popularity Ranking)**：文案最末尾**强制输出 6-10 个**以 `#` 开头的相关话题标签。拟定标签时**必须深入结合本文核心主题以及当前各社交平台（小红书/微信/微博/知乎等）的相关话题热度**，且**必须严格按照热度与覆盖范围从高到低衰减进行降序排列**（排列顺序：全网/全行业高热大词 ➔ 垂直领域热门标签 ➔ 本文核心概念细分标签，如 `#AI大模型 #人工智能 #LLM原理 #Agent架构 #状态机`）。

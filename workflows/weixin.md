...
2. **调度原子技能 `wx-formatter` 格式化重构**：
   - 检查 `./<article-slug>/assets/cover.md` 是否存在。若不存在或需更新，自动调度原子技能 `cover-designer`（`platform: weixin`），提炼微信公众号 2.35:1 爆款头图封面配置文件。
...
4. **离线 HTML 存盘交付与人工确认提示**：
   ...
   - **统一人工确认提示**：
     ...
     > 2. 如对本次生成的 CSS 样式或卡片元素进行了人工修正，请在对话框回复 **`/workflow-learn`**，系统将自动提炼您的微信排版偏好并沉淀落盘，让后续盲审标准自动进化！

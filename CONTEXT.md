...
**单视频单元独立渲染 (Per-Unit Independent Rendering)**:
将长视频拆分为按知识结构划分的视频单元（`unit_XX`），使用 HyperFrames 独立导出单视频单元 MP4 片段，解耦全量合成的内存瓶颈。视频单元内部的分镜设计与 3 幕动作链落盘于该单元的 `BRIEF.md` 中。
_Avoid_: 全量合成, 长视频一次性渲染, 硬编码固定短分镜
